...
async def on_message(message):
    ...
    if message.content.startswith("D-출근"):
        ...
        for i in message.author.roles:
            ...
            if i.name == '[ 디제이에스 ] 부관리자':
                ...
                await client.get_channel(int(725131942655426630)).send(embed=cnfrms1)
            if i.name == '[ 디제이에스 ] 대리인':
                ...
                await client.get_channel(int(725967139915169822)).send(embed=cnfrms2)
            if i.name == '[ 디제이에스 ] 임시대리인':
                ...
                await client.get_channel(int(725967139915169822)).send(embed=cnfrms3)
                break
                    
    if message.content.startswith("D-퇴근"):
        await message.delete()
        for i in message.author.roles:
            if i.name == '[ 디제이에스 ] 관리자':
            ...
            if i.name == '[ 디제이에스 ] 부관리자':
                ...
                await client.get_channel(int(725131942655426630)).send(embed=xhlrms1)
            if i.name == '[ 디제이에스 ] 대리인':
                ...
                await client.get_channel(int(725967139915169822)).send(embed=xhlrms2)
            if i.name == '[ 디제이에스 ] 임시대리인':
                ...
                await client.get_channel(int(725967139915169822)).send(embed=xhlrms3)
                break
                
    if message.content.startswith("D-배너"):
        await message.delete()
        for i in message.author.roles:
            if i.name == '[ 디제이에스 ] 배너직원':
                b = message.content[5:].split(" ")
                bcn = b[0]
                bf = b[2]
                bi = bf[3:21]
                bci = client.get_channel(int(b[1]))
                await bci.edit(name=bcn)
                ba = message.guild.get_member(int(bi))
                role = discord.utils.get(message.guild.roles, name="[ 디제이에스 ] 맞배너사장님")
                bag = client.get_user(int(bi))
                be = discord.Embed(title="**( 봇 ) 디제이에스**", color=0x8affc6)
                be.add_field(name="배너알림", value=f"**```\n( {bcn} ) 의 배너가 완료되었습니다.```**", inline=False)
                be.set_thumbnail(url=message.author.avatar_url)
                be.set_footer(text="djs의 gta5 대리" , icon_url="https://media.discordapp.net/attachments/711019332477517856/733254434292891709/djsqht.png")
                await bag.send(embed=be)
                await ba.add_roles(role)
                await bci.send(f"**<@{bi}> 님 배너가 완료되었습니다.**")
                break
